...
function getCardValue(item: DsItem, key: string): string {
  ...
  if (["Date DS","Date entrée","Date interv","Effectué le"].includes(key)) return fmtDate(v as string);
...
function labelValueRow(label: string, value: string, shade?: string): TableRow {
  return new TableRow({
    children: [
      new TableCell({
        borders,
        ...
        children: [new Paragraph({ children: [new TextRun({ text: label, bold: true, size: 18, color: COLORS.labelFg, font: "Arial" })] })],
      }),
      new TableCell({
        borders,
...
function sectionHeading(text: string): Paragraph {
  return new Paragraph({
    ...
    border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: COLORS.sectionFg, space: 1 } },
    children: [new TextRun({ text, bold: true, size: 20, color: COLORS.sectionFg, font: "Arial" })],
  });
}

function buildVehicleTable(item: DsItem, fields: { key: string; label: string }[]): Table {
  return new Table({
    width: { size: PAGE_W, type: WidthType.DXA },
    columnWidths: [2200, PAGE_W - 2200],
    rows: fields.map(f => labelValueRow(
      f.label,
      (() => { const v = (item as Record<string,unknown>)[f.key]; return v != null ? String(v).trim() || "—" : "—"; })()
    )),
  });
}

function buildDsInfoTable(item: DsItem, fields: string[], labels: Record<string, string>): Table {
  return new Table({
    width: { size: PAGE_W, type: WidthType.DXA },
    columnWidths: [2200, PAGE_W - 2200],
...
function buildLinesTable(lines: Line[], fields: string[], labels: Record<string, string>, totalMtHt?: number | null): Table {
  const numKeys = new Set(["qte","mt_ht","prix_unitaire","dernier_prix"]);
  const colW = Math.floor(PAGE_W / fields.length);
  const colWidths = fields.map((_, i) => i === fields.length - 1 ? PAGE_W - colW * (fields.length - 1) : colW);

  const headerRow = new TableRow({
    tableHeader: true,
    children: fields.map((key, i) => new TableCell({
      borders,
      ...
      children: [new Paragraph({
        ...
        children: [new TextRun({ text: labels[key] ?? key, bold: true, size: 18, color: COLORS.headerFg, font: "Arial" })],
      })],
    })),
  });

  const dataRows = lines.map((line, rowIdx) => new TableRow({
    children: fields.map((key, i) => new TableCell({
      borders,
      width: { size: colWidths[i], type: WidthType.DXA },
...
export async function POST(req: Request) {
  ...
  const {
    ...
    vehicleMetaFields, cardFieldLabels, lineFieldLabels, topBarKeys,
  } = payload;
  ...
  // Fields to show in the DS info table (exclude top-bar & N°DS — already in heading)
  ...
  children.push(new Paragraph({
    ...
    children: [new TextRun({ text: `Historique DS — ${immat}`, bold: true, size: 36, color: COLORS.headerBg, font: "Arial" })],
  }));
  children.push(new Paragraph({
    spacing: { after: 300 },
    children: [new TextRun({ text: `Généré le ${now}  ·  ${count} dossier${count > 1 ? "s" : ""}`, size: 18, color: "888888", font: "Arial" })],
  }));
  ...
  items.forEach((it, idx) => {
    ...
    if (it.lines?.length && visibleLineFields.length > 0) {
      children.push(new Paragraph({ spacing: { before: 140, after: 60 }, children: [
        new TextRun({ text: `Lignes (${it.lines.length})`, bold: true, size: 18, color: COLORS.sectionFg, font: "Arial" }),
      ]}));
    ...
    styles: {
      ...
      paragraphStyles: [
        ...
        },
        {
          ...
          run: { size: 26, bold: true, font: "Arial", color: COLORS.headerBg },
          paragraph: { spacing: { before: 400, after: 80 }, outlineLevel: 1 },
        },
      ],
    },
    sections: [{
