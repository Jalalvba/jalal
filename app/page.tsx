...
type DsHistoryItem = {
  "N°DS": string;
  Societe?: string; Site?: string; "SITE DS"?: string;
  "Date DS"?: string; "Date entrée"?: string; "Date interv"?: string; "Effectué le"?: string;
  Immatriculation?: string; Parc?: string; "Type Parc"?: string;
  "Désignation véhicule"?: string; Marque?: string;
  ENTITE?: string; "Code entité"?: string; Entité?: string;
  Description?: string; "Type DS"?: string; "Type de DS"?: string;
  Techniciens?: string[];
  User?: string | null; "Facturé par"?: string | null;
  "Client Final"?: string; "Raison Social"?: string; "Client DS"?: string;
  "Code Client"?: string; "Détenteur DS"?: string; "Détenteur parc"?: string;
  "Locat Parc"?: string; "A Facturé"?: string; "Statut facture"?: string;
  "N° Facture"?: string; Affectation?: string; "Ref CP"?: string; "CMD Num"?: string;
  Réceptionné?: string; Soldé?: string; "Demande satisfaite"?: string; Fournisseur?: string;
  KM?: number; "MT Total HT"?: number; lines?: Line[];
};

type ApiResponse = { ok: boolean; imm: string; count: number; items: DsHistoryItem[]; error?: string };

// ─── Field definitions ────────────────────────────────────────────────────────

type MetaField = { key: keyof DsHistoryItem; label: string };
...
const VEHICLE_META_FIELDS: MetaField[] = [
  ...
  { key: "Désignation véhicule", label: "Désignation véhicule" },
  { key: "Marque",               label: "Marque" },
  { key: "Parc",                 label: "Parc" },
  { key: "Type Parc",            label: "Type Parc" },
  { key: "Societe",              label: "Société" },
  { key: "Détenteur parc",       label: "Détenteur parc" },
  { key: "Locat Parc",           label: "Locat Parc" },
];
...
const CARD_FIELDS: CardField[] = [
  ...
  { key: "CMD Num",            label: "CMD Num",            group: "Facturation" },
...
function buildPdfHtml(
  data: ApiResponse, vehicle: DsHistoryItem,
  orderedCardFields: CardField[], orderedLineFields: LineField[],
...
<title>Historique DS — ${vehicle.Immatriculation??""}</title>
<style>
...
<h1>Historique DS — ${vehicle.Immatriculation??""}</h1>
<p style="color:#888;font-size:10px;margin:0 0 14px">Généré le ${now} · ${data.count} dossier${data.count>1?"s":""}</p>
<h2>Véhicule</h2>
<table style="border-collapse:collapse;margin-bottom:16px;width:500px">${vehicleRows}</table>
...
async function downloadDocx(
  ...
  try {
    ...
    const res = await fetch("/api/ds/export", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        imm: vehicle.Immatriculation ?? "",
        count: data.count,
        items: data.items,
    ...
    a.href = url;
    a.download = `historique_ds_${(vehicle.Immatriculation??"ds").replace(/[^a-zA-Z0-9-]/g,"_")}.docx`;
    a.click();
...
function VehicleMetaBar({ item }: { item: DsHistoryItem }) {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="flex items-center gap-2 border-b border-zinc-100 px-5 py-3 dark:border-zinc-800">
        ...
        <span className="text-xs font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Véhicule</span>
        <span className="ml-auto text-xs italic text-zinc-400 dark:text-zinc-600">Données fixes — non modifiées par les DS</span>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-end" onClick={onClose}>
      <div className="mr-4 mt-16 w-80 max-h-[80vh] overflow-y-auto rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-700 dark:bg-zinc-950"
        ...
        <div className="mx-3 mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700 dark:border-amber-800/40 dark:bg-amber-950/30 dark:text-amber-400">
          🚗 Les champs <b>Véhicule</b> sont dans la barre de métadonnées, pas dans les DS.
        </div>
...
export default function Home() {
  ...
  async function fetchHistory(nextImm?: string) {
    ...
    try {
      ...
      qs.set("imm", immVal); qs.set("limit", String(limit));
      if (year) qs.set("year", year);
      const res  = await fetch(`/api/ds/history?${qs}`);
      const json = (await res.json()) as ApiResponse;
      if (!res.ok || !json.ok) { setData(null); setError(json?.error || `Erreur (${res.status})`); }
      else setData(json);
    } catch (e) { setData(null); setError(e instanceof Error ? e.message : "Unknown error"); }
    finally     { setLoading(false); }
  }

  useEffect(() => { fetchHistory("48070-B-7"); }, []); // eslint-disable-line

  const vehicleMeta       = data?.items?.[0] ?? null;
  const orderedCardFields = CARD_FIELDS.filter(f => visibleCardFields.has(f.key) && f.key !== "N°DS" && !TOP_BAR_KEYS.has(f.key as string));
  const orderedLineFields = LINE_FIELDS.filter(f => visibleLineFields.has(f.key));

  function handlePdf() {
    if (!data || !vehicleMeta) return;
    printAsPdf(buildPdfHtml(data, vehicleMeta, orderedCardFields, orderedLineFields, visibleCardFields));
  }

  function handleDocx() {
    if (!data || !vehicleMeta) return;
    downloadDocx(data, vehicleMeta, visibleCardFields, visibleLineFields, setExportingDocx);
  }
      ...
      <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Recherche par immatriculation</p>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          <div className="grid gap-3 sm:grid-cols-12 sm:items-end">
            <div className="sm:col-span-5">
              <label className="mb-1 block text-xs font-medium text-zinc-500 dark:text-zinc-400">Immatriculation</label>
              <input value={imm} onChange={e => setImm(e.target.value)}
                onKeyDown={e => e.key==="Enter" && fetchHistory()} placeholder="ex: 48070-B-7"
            ...
            <div className="sm:col-span-2">
              <button onClick={() => fetchHistory()} disabled={loading || !imm.trim()}
        ...
        {/* Vehicle metadata */}
        {vehicleMeta && !loading && <div className="mt-4"><VehicleMetaBar item={vehicleMeta} /></div>}
            ...
            <div key={it["N°DS"]} className="rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 px-5 py-3 dark:border-zinc-800">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {visibleCardFields.has("Date DS") && (<><span className="text-zinc-400">•</span><span className="text-sm text-zinc-500">{fmtDate(it["Date DS"])}</span></>)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  {visibleCardFields.has("KM") && it.KM != null && (
                    <span className="text-xs tabular-nums text-zinc-500">{fmtNum(it.KM)} km</span>
                  )}
                </div>
              </div>
              ...
              {it.lines?.length && orderedLineFields.length > 0 ? (
                <div className="border-t border-zinc-100 px-5 pb-4 pt-3 dark:border-zinc-800">
                  ...
                  <LinesTable lines={it.lines} orderedLineFields={orderedLineFields}
                    totalMtHt={it["MT Total HT"]} visibleLineFields={visibleLineFields} />
                </div>
              ) : null}
