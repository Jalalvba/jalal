...
**Primary files:**
[`app/api/trigger-import/route.ts`](../app/api/trigger-import/route.ts) ·
...
([`route.ts:52-56`](../app/api/import-status/route.ts#L52)), which validates
against **`KNOWN_RUN_STATUSES`**
([`route.ts:41-50`](../app/api/import-status/route.ts#L41)) — the run-level
...
> `app/api/import-status/__tests__/route.test.ts` only exercised
> `status: "success"`.
>
...
Two distinct enums in [`lib/types.ts`](../lib/types.ts), easy to conflate (and
...
`lib/pipeline_log.py`'s `PipelineLogger.to_document()` **directly** — not
...
A button on the home page (`app/page.tsx`) driving a terminal-style replay.
...
wired into [CI](../.github/workflows/ci.yml)) scans every `app/api/**` file's
