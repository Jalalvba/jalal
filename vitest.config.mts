...
export default defineConfig({
  test: {
    ...
    env: {
      // src/lib/sheets/googleSheetsClient.ts throws at module load if this is unset —
      ...
      // Same reasoning — src/lib/mongo/client.ts throws at module load if these are
      // unset. Any route/module that transitively imports src/lib/http/rateLimit.ts
      // (which imports src/lib/mongo/client.ts) needs this just to be importable, even
      ...
      // src/lib/sheets/googleSheetsBdd.ts (and the other src/lib/sheets/googleSheets*.ts modules)
      ...
      // iron-session (src/lib/auth/session.ts) requires a real >=32-char secret at
      // seal/unseal time — used by src/lib/__tests__/proxy.test.ts to mint a
      // real sealed session cookie the same way e2e/helpers/auth.ts does
      // for the Playwright suite, so src/proxy.ts's actual iron-session
