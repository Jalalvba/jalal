...
// ── Complaint handler — Phase 1 (playbook generation) ──────────────────────
// Phase 2 (applying a playbook to a new incoming complaint) is not built yet;
// nothing here is scaffolding for it.

export type PlaybookConfidence = "high" | "medium" | "low";
export type PlaybookEffectivenessVerdict =
  | "effective"
  | "ineffective"
  | "mixed"
  | "not-determinable";
...
export type ComplaintCategory = {
  /** kebab-case slug — the stable key Phase 2 would look a category up by. */
  id: string;
  label: string;
  description: string;
  evidence: {
    threadCount: number;
    clientGoalsObserved: string[];
  };
  avisResponsePattern: {
    tone: string;
    typicalConcessions: string[];
    typicalTimeline: string | null;
    escalationPath: string | null;
    channel: string;
  };
  effectiveness: {
    verdict: PlaybookEffectivenessVerdict;
    signals: string[];
  };
  recommendedResponse: {
    openingMove: string;
...
/**
 * What the model returns, plus the metadata added server-side. `notEvidenced`
 * is load-bearing, not decorative: it is where the model records what the
 * uploaded threads do NOT establish, so a reader can tell a real finding from
 * an absent one.
 */
export type ComplaintPlaybook = {
  schemaVersion: number;
  sourceSummary: {
    threadsObserved: number;
    dateRangeObserved: string | null;
...
/** Metadata about the uploaded file. Deliberately excludes the raw text itself. */
export type PlaybookSourceMeta = {
  filename: string;
  charCount: number;
  /** Rough count shown to the user before submitting; the model reports its own. */
  estimatedThreadCount: number;
  /** sha256 of the upload, so a re-run of the same file is recognisable without storing it. */
  contentHash: string;
};

export type StoredComplaintPlaybook = ComplaintPlaybook & {
  generatedAt: string;
  model: string;
  source: PlaybookSourceMeta;
};

export type GeneratePlaybookRequest = {
  filename: string;
  content: string;
};

export type GeneratePlaybookResponse =
  | {
      ok: true;
