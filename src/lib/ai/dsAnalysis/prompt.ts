...
export const DS_GROUNDING_RULES: readonly string[] = [
  ...
  "2d. LES SYMPTÔMES RÉPÉTÉS COMPTENT AUSSI, même sans pièce associée. Certaines pannes n'apparaissent QUE dans les descriptions : « PB FAP », « témoin moteur allumé », « manque de puissance », « bruit moteur », « PB DEMARRAGE ». Si un même symptôme ou un même organe revient 2 fois ou plus À TRAVERS LES DESCRIPTIONS — y compris quand l'intervention ne porte aucune pièce — produis un constat dédié en citant les dates, et rattache-lui les interventions liées (diagnostic, nettoyage, additif) qui portent sur le même organe. Exemple réel manqué : trois interventions FAP (régénération, diagnostic « PB FAP », additif FAP) réparties sur un an, suivies d'une entrée récente « manque de puissance, témoin de défaut moteur » — c'est le motif d'immobilisation du véhicule, et il n'était visible que dans les descriptions.",
  "2e. La DERNIÈRE intervention de la liste est la plus récente. Si elle ne porte aucune pièce et décrit un symptôme, c'est le problème EN COURS, non résolu : signale-le en priorité et relie-le à l'historique quand celui-ci l'éclaire.",
  ...
  "4. Les descriptions sont des notes d'atelier très brèves, souvent vides de sens (« pb », « . »). Les désignations de pièces sont le signal le plus fiable pour dire CE QUI A ÉTÉ FAIT : appuie-toi dessus en priorité et n'extrapole jamais une panne à partir d'une description pauvre. Cela ne veut PAS dire d'ignorer les descriptions : une description qui nomme clairement un organe ou un symptôme est une donnée, et sa RÉPÉTITION est un signal à part entière (règle 2d). Ce qui est interdit, c'est de broder sur une description vide, pas de lire celles qui parlent.",
  ...
  "16b. Si les données indiquent « Propriétaire : AVIS », ce véhicule appartient au parc propre d'AVIS (location courte durée) et non à un client : dis-le une fois, explicitement, dans le résumé. C'est une information de gestion, pas un constat technique — ne l'utilise pas pour juger l'entretien. Si l'information n'est pas fournie, n'en parle pas et ne devine pas le propriétaire.",
