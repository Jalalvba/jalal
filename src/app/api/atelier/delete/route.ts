...
import { deleteAtelierRow } from "@/lib/sheets/googleSheetsAtelier";
import { rateLimitOrNull } from "@/lib/http/rateLimit";
import { toErrorResponse } from "@/lib/http/apiError";

export async function POST(req: Request) {
  const limited = await rateLimitOrNull(req, "atelier-delete", 30, 60_000);
  ...
  try {
    await deleteAtelierRow(rowIndex, imm);
