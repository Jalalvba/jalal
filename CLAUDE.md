...
| DS History | `src/app/ds-history/page.tsx`, `/api/ds/history`, `/api/parc`, `/api/cp`, `/api/query*`, `/api/sheet` | Plate-only Mongo search (`ds`/`bc`) + `VehicleCard` + `SheetCard` (BDD/RL/Import tabs). Vehicle identity is `parc` **merged with** `cp` via `src/lib/vehicle/identity.ts` — `parc` wins per field, `cp` fills gaps, and the card renders whenever *either* source has data (requiring `parc` hid it on 46.6% of plates). The same identity feeds the PDF/DOCX exports and the AI payload. See [`docs/ds-history.md`](./docs/ds-history.md) §1.5. |
